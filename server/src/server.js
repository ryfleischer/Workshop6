...
// `POST /comment { feedItemId: feedItemId, author: author, contents: contents  }`
app.post('/feeditem/:feeditemid/comments',
  ...
  if (fromUser === body.author) {
    var newUpdate = postComment(req.params.feeditemid, body.author, body.contents);
    // When POST creates a new resource, we should tell the client about it
    // in the 'Location' header and use status code 201.
    res.status(201);
    res.set('Location', '/feeditem/' + newUpdate._id + '/comments');
     // Send the update!
    res.send(newUpdate);
  } else {
    // 401: Unauthorized.
...
app.put('/feeditem/:feeditemid/comments/:commentid/likelist/:userid', function(req, res) {
  ...
  if (fromUser === userId) {
    ...
    if (feedItem.comments[commentId].likeCounter.indexOf(userId) === -1) {
      ...
      writeDocument('feedItems', feedItem);
      feedItem.comments[commentId].author = readDocument('users', userId)
    }
    // Return a resolved version of the likeCounter
